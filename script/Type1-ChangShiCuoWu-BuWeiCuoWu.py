                    ...
                    for k, err in enumerate(errs):
                        if err == word:
                            ...
                            prompt = (
                                    ...
                                    f"上文：{context_upper}\n" +
                                    f"下文：{context_lower}\n" +
                                    f"这段文本来自于研报、招标书或者法律条文，你需要判断以下这组相互矛盾的句子中哪个不符合上下文语义：" +
                                    f"句子序号1：{possible_error_sent}\n" +
                                    f"句子序号2：{possible_error_sent_antonymy}\n" +
                                    """
                                    请综合上述信息，你给出的回复需要包含以下这两个字段：
                                    1.num: 不符合段落语义的句子的序号
                                    2.error_sentence: 指出不符合段落语义的那个句子，输出包含错误句法的最小粒度分句，请用 markdown 格式。
                                    请按照以下JSON格式来回答：
                                    {
                                        "num": [
                                            "<你输出的有错误的句子序号>"
                            ...
                            messages = [
                                {"role": "system", "content": "作为一位识别金融文本中的漏洞和矛盾的专家，您的任务是对一组矛盾的句子进行判断，选出不符合上下文语义的那个句子。"},
